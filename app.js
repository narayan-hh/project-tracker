...
const NAV = [
  ...
  { href:'#/review',    label:'Weekly Review', icon:'M3 3v18h18M7 15l4-4 3 3 5-6' },
...
function render(){
  ...
  else if(h[0] === 'review')               html = viewReview();
...
function runAction(a, act){
  ...
  if(a === 'add-task'){
    const t = { id:uid('t'), month:thisMonth(), name:'New task',
      status:'todo', priority:Number(act.dataset.pri) || 1, due:'', note:'', doneAt:'' };
  ...
  if(a === 'add-entry'){
    const b = { id:uid('b'), date:today(), title:'New expense',
      amount:0, vendor:'', by:'', head:DB.budget.heads[0] || 'Other', approvedBy:'' };
...
document.addEventListener('change', e => {
  ...
  if(el.matches('[data-act="set-field"]') || el.matches('[data-act="set-task-status"]')
     || el.matches('[data-act="set-date"]')){
    setPath(el.dataset.path, el.value, el.dataset.num === '1');

    /* a task just marked complete gets today's date against it, so the
       weekly review can show what actually moved */
    if(el.dataset.act === 'set-task-status'){
      const m = String(el.dataset.path).match(/^myTasks#([^.]+)\./);
      const t = m && DB.myTasks.find(x => x.id === m[1]);
      if(t) t.doneAt = t.status === 'done' ? (t.doneAt || today()) : '';
    }
    save(); render(); return;
  }
